...
@Component({
  ...
  template: `
    @if (storeContext.loading()) {
      ...
      <div class="min-h-screen flex items-center justify-center bg-theme-surface px-4">
        <div class="flex flex-col items-center gap-4">
          <div class="w-12 h-12 sm:w-14 sm:h-14 border-4 border-theme-secondary border-t-gray-800 rounded-full animate-spin"></div>
          <p class="text-theme-muted animate-pulse text-sm sm:text-base">Loading store...</p>
        </div>
      </div>
    } @else if (storeContext.error()) {
      <!-- Error State - Mobile friendly -->
      <div class="min-h-screen flex flex-col items-center justify-center bg-theme-surface px-4 py-10">
        <div class="text-center max-w-sm w-full">
          ...
          <h1 class="text-xl sm:text-2xl font-bold text-theme-fg mb-2">Store Not Found</h1>
          <p class="text-theme-muted mb-6 text-sm sm:text-base">{{ storeContext.error() }}</p>
          <a [href]="subdomainService.getMainSiteUrl()" class="inline-flex items-center justify-center w-full sm:w-auto px-8 py-3.5 bg-theme-fg text-white font-semibold rounded-xl hover:bg-theme-surface-hover active:bg-theme-surface-hover transition-colors min-h-[48px]">
            Visit Main Site
          </a>
        </div>
      </div>
      ...
      <div class="min-h-screen flex flex-col bg-theme-surface font-sans antialiased">
        ...
        <header class="bg-white/95 backdrop-blur-md border-b border-theme-secondary sticky top-0 z-40 safe-area-top">
          <div class="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
            <div class="flex items-center justify-between h-14 sm:h-16">
              ...
              >
                @if (storeContext.store()?.logoUrl) {
                  ...
                  <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-[#b8e6c9] to-[#d8f8e0] flex items-center justify-center shadow-sm">
                    <span class="text-base sm:text-lg font-bold text-theme-fg">
                      {{ storeContext.storeName().charAt(0).toUpperCase() }}
                    </span>
                  </div>
                }
                <span class="font-bold text-theme-fg text-sm sm:text-base truncate max-w-[120px] sm:max-w-[200px]">
              ...
              <div class="flex items-center gap-1 sm:gap-2">
                ...
                <button 
                  ...
                  class="hidden sm:flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-theme-muted hover:text-theme-fg hover:bg-theme-secondary rounded-lg transition-colors"
                >
                ...
                <button 
                  ...
                  class="sm:hidden flex items-center justify-center w-11 h-11 rounded-xl hover:bg-theme-secondary active:bg-theme-secondary transition-colors"
                  aria-label="Menu"
                >
                ...
                <button 
                  ...
                  class="relative flex items-center justify-center w-11 h-11 sm:w-12 sm:h-12 rounded-xl hover:bg-theme-secondary active:bg-theme-secondary transition-colors -mr-1"
                  aria-label="Open cart"
                >
                  <svg class="w-6 h-6 text-theme-fg" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/>
                  </svg>
                  @if (cartService.itemCount() > 0) {
                    <span class="absolute top-0.5 right-0.5 sm:top-1 sm:right-1 min-w-[20px] h-5 px-1.5 bg-theme-fg text-theme-bg text-xs font-bold rounded-full flex items-center justify-center animate-scale-in">
            ...
            @if (mobileMenuOpen()) {
              <div class="sm:hidden border-t border-theme-secondary py-2">
                <button 
                  (click)="handlePurchasesClick()"
                  class="w-full flex items-center gap-3 px-2 py-3 text-sm font-medium text-theme-fg hover:bg-theme-bg rounded-lg"
                >
        ...
        <footer class="bg-gradient-to-br from-gray-50 to-white border-t border-theme-secondary safe-area-bottom">
          <div class="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8">
              ...
              <div class="flex items-center gap-2.5 sm:gap-3">
                @if (storeContext.store()?.logoUrl) {
                  ...
                  <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-[#b8e6c9] to-[#d8f8e0] flex items-center justify-center">
                    <span class="text-sm font-bold text-theme-fg">
                      {{ storeContext.storeName().charAt(0).toUpperCase() }}
                    </span>
                  </div>
                }
                <span class="font-bold text-theme-fg text-sm sm:text-base">{{ storeContext.storeName() }}</span>
              </div>
            ...
            <!-- Copyright - Mobile -->
            <div class="mt-5 sm:mt-6 pt-5 sm:pt-6 border-t border-theme-secondary text-center">
